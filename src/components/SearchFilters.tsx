...
const SCHEDULE_OPTIONS = [
  { id: "all", label: "All Schedule", isParent: true, level: 0 },
  { id: "scheduled", label: "Scheduled", isParent: true, level: 1 },
  { id: "weekly", label: "Weekly", isParent: false, level: 2 },
  { id: "monthly", label: "Monthly", isParent: false, level: 2 },
  { id: "manually", label: "Manually", isParent: true, level: 1 }
];
...
const SearchFilters = () => {
  ...
  const [selectedTimePeriod, setSelectedTimePeriod] = useState("Last 2 weeks");
    ...
    <div className="bg-white py-4">
      <div className="flex items-center gap-4 w-full mb-4">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="flex items-center gap-2">
              {SCHEDULE_OPTIONS.find(opt => opt.id === selectedSchedule)?.label || "All Schedule"}
              <ChevronDown className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-56 bg-white border shadow-lg z-50 rounded-xl p-2">
            {SCHEDULE_OPTIONS.map((option) => (
              <DropdownMenuItem
                key={option.id}
                onClick={() => handleScheduleToggle(option.id)}
                className={cn(
                  "cursor-pointer flex items-center gap-3 rounded-lg px-3 py-2",
                  option.level === 2 && "ml-6",
              ...
              >
                <Checkbox
                  ...
                  className={cn(
                    ...
                    selectedSchedule === option.id
                      ? "data-[state=checked]:bg-white data-[state=checked]:border-white data-[state=checked]:text-blue-500"
                      : "border-gray-300"
                  )}
                />
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Badge
          variant="outline"
          className="px-3 py-1 bg-[#E9F0FA] text-foreground border-border-grey rounded-md flex items-center gap-2"
        >
        ...
        {selectedKeywords.map((keyword) => (
          <Badge
            key={keyword}
            variant="outline"
            className="px-3 py-1 bg-[#E9F0FA] text-[12px] text-foreground border-border-grey rounded-md flex items-center gap-2"
          >
            <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
